...
| 日期 | 文件 | 优化内容 | 验证证据 | 状态 |
|---|---|---|---|---|
| 2026-08-31 | `frontend/src/components/AnalysisWorkspaceCard.vue`、`frontend/src/components/CaseWorkspaceControls.vue`、`frontend/src/pages/CaseWorkspacePage.vue`、`frontend/e2e/desktop.real.mjs` | 收敛 r17/r19 桌面按钮点击后的 UI 跳动：分析卡片内部改为固定高度独立滚动区，桌面工作台保留稳定的分析与证据槽位，操作状态文本固定高度并安全截断，临时实时服务忙/限流提示改为稳定信息态；真实审计仅比较持久化外壳锚点，避免把模式内容替换误报为页面位移。 | `npm --prefix frontend run typecheck`、前端 Vitest 56 文件/252 项、`npm run desktop:test` 6 项通过；r21 运行包完整性校验通过；243 个按钮真实点击审计 0 失败、0 并发/限流错误、布局位移检查通过、浏览器错误 0；窗口关闭后端口与进程树释放。结果：`output/playwright/desktop-real-test/20260831081255-44272/`。 | 已完成 |
...
| 2026-08-30 | `osteo_vision_core/core/paths.py`、`tests/unit/test_core_paths.py`、`frontend/e2e/desktop.real.mjs`、`scripts/run_desktop_real_test.ps1` | 增加 `OSTEO_PROJECT_ROOT` 对 PyInstaller 运行包相对模型、配置和产物路径的统一解析；新增桌面 EXE 真实 Playwright 测试入口，覆盖启动、内置 OFDVDnet、双通道、人工标注、D024、MP4、摄像头模拟和退出清理，失败证据写入 `output/playwright/desktop-real-test/`；测试对实时帧容量保护 `429` 与标准示例初始化竞争 `409` 做端点级解释，仍报告其他 HTTP 错误。 | `node --check frontend/e2e/desktop.real.mjs`、`pytest -q tests/unit/test_core_paths.py backend/tests/unit/test_desktop_runtime_shutdown.py`（4 项）、`npm run desktop:test`（6 项）通过；重打包后 `npm run desktop:real-test -- -TimeoutMs 180000` 与 `--DisableGpu` 均通过，覆盖桌面窗口、后端 `/ready`、无全局运行状态横幅、OFDVDnet 三视图同步、双通道、人工标注、D024 STL、MP4 播放、模拟摄像头实时分割、退出端口及进程树清理。结果目录：`output/playwright/desktop-real-test/20260830065102-39228/`、`output/playwright/desktop-real-test/20260830065136-23500/`。 | 已完成 |
...
- 2026-08-30 桌面真实自动化验证：`npm run desktop:real-test -- -TimeoutMs 180000` 与 `npm run desktop:real-test -- -DisableGpu -TimeoutMs 180000` 均通过；两轮均验证 EXE 启动、内置 OFDVDnet 三视图、文件双通道、人工标注、D024、MP4 导入/播放、模拟摄像头实时分割和退出清理。实时帧容量保护出现的 `429` 与标准示例初始化竞争 `409` 均限定在对应端点并纳入解释性检查，未发现其他浏览器网络错误。
- 2026-08-30 离线发行包运行包复核：生成 `artifacts/release/offline-release/Osteo-Vision-Offline-Release-win32-x64-20260830-r5/`，清单校验通过，包体 `5,454,350,066` 字节（低于 8.5 GB 配置上限）；使用该目录执行完整桌面真实测试通过，结果 `output/playwright/desktop-real-test/20260830070004-32480/`（21 项检查、0 失败、0 浏览器错误），证明光盘内置示例数据、桌面 EXE、双通道、摄像头实时分割与退出清理可用。
...
- 当前严格运行闭环复跑通过：严格配置和 checkpoint 校验通过，4K JPEG 融合、4K MP4 关键帧主线模型、工程复核、51 项病例产物与 40.9 MB 证据包均成功生成，模型回退未触发。
