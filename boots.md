...
- 2026-08-30 比赛光盘运行包复核：生成 `artifacts/release/competition-disc/Osteo-Vision-Competition-Disc-win32-x64-20260830-r5/`，清单校验通过，包体 `5,454,350,066` 字节（低于 8.5 GB 配置上限）；使用该目录执行完整桌面真实测试通过，结果 `output/playwright/desktop-real-test/20260830070004-32480/`（21 项检查、0 失败、0 浏览器错误），证明光盘内置示例数据、桌面 EXE、双通道、摄像头实时分割与退出清理可用。
