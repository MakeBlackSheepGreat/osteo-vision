...
`keyframe_residual_attention_unet_s20260715_20260715` 已晋级为比赛版平台的 JPEG/MP4 keyframe 分割主线。严格配置、研发配置和任务包已同步，上一版 `convnext2d_keyframe_proxy_segmenter` 保留为研发对照。
...
- 晋级后的 4K 完整比赛流通过：`3840x2160` JPEG/MP4、2/2 keyframe probability map、医生工程复核、证据包导出均完成，未触发启发式回退。
...
- 严格比赛配置显式设置 `allow_heuristic_keyframe_fallback=false`；模型不可用时分析任务进入结构化失败状态。
...
| 晋级后 4K 完整比赛流 | 通过 | `b51dc0d7df0d668bc6da2fe8eebf1d1a88fb10775a0a7780285ae0c4a2efe46b` |
