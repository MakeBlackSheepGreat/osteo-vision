param(
    ...
    [string]$ReleaseName = "Osteo-Vision-Offline-Release-win32-x64",
...
$releaseRoot = Join-Path $repoRoot "artifacts\release\offline-release"
$packageRoot = Join-Path $releaseRoot $ReleaseName
$stagingRoot = Join-Path $releaseRoot "$ReleaseName.staging"
$discAssets = Join-Path $repoRoot "packaging\offline_release"
...
foreach ($relativePath in $requiredFiles) {
    ...
    if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
        throw "Platform disc layout is missing required file: $relativePath"
    }
}
...
$manifest = [ordered]@{
    schema_version = 1
    package_id = "osteo-vision-offline-release"
...
Write-Host "Platform disc package created at $packageRoot"
