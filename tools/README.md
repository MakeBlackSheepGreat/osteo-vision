...
| 证据与报告 | `build_evidence_index.py`、`run_platform_flow_demo_check.py` |
| 性能与压力 | `benchmark_core_hotpaths.py`、`run_official_4k_pressure_smoke.py`、`run_fusion_performance_gate.py`、`run_task2_temporal_registration_gate.py`、`run_keyframe_live_fast_output_gate.py` |
...
- `build_evidence_index.py`：根据平台证据 manifest 生成版本化 JSON/Markdown 索引，记录文件存在性、SHA256、模型清单与离线发布状态。
...
`run_fusion_performance_gate.py` 的 100 ms 门只统计任务2配准估计与变换、GPU 归一化、伪彩和 Alpha 融合；AI 推理、文件解码、证据编码、磁盘写入和网络传输保留独立阶段指标。默认排除一次预热并执行 10 次 4K 计时。
