...
def choose_threshold(y_true: list[int], y_score: list[float], strategy: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "type": "fixed",
        "threshold": threshold,
        "analysis": {"available": bool(y_true), "metrics": classification_metrics(y_true, y_score, threshold) if y_true else {}},
    }
...
def _sensitivity_first(y_true: list[int], y_score: list[float], strategy: dict[str, Any]) -> dict[str, Any]:
    ...
    if not viable:
        return {"type": "sensitivity_first", "threshold": 0.5, "analysis": {"available": False, "reason": "no threshold meets target sensitivity"}}
    best = max(viable, key=lambda item: (item["metrics"]["specificity"], item["metrics"]["f1"]))
    return {"type": "sensitivity_first", "threshold": best["threshold"], "analysis": {"available": True, "target_sensitivity": target, "best": best}}
