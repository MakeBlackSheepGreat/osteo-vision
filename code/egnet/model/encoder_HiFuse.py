...
from model.lib.decoders import EMCAD
...
class main_model(nn.Module):
    ...
    def __init__(self, num_classes=1, patch_size=4, in_chans=3, embed_dim=96, depths=(2, 2, 18, 2),
                 ...
                 conv_depths=(2, 2, 18, 2), conv_dims=(96, 192, 384, 768), conv_drop_path_rate=0.,
        ...
        self.decoder = EMCAD(channels=[768,384,192,96], kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, lgag_ks=3, activation='relu')
    ...
    def forward(self, imgs):
        ...
        x_s_1 = x_s_1.view(x_s_1.shape[0], -1, 64, 64)
        x_s_2 = torch.transpose(x_s_2, 1, 2)
        x_s_2 = x_s_2.view(x_s_2.shape[0], -1, 32, 32)
        x_s_3 = torch.transpose(x_s_3, 1, 2)
        x_s_3 = x_s_3.view(x_s_3.shape[0], -1, 16, 16)
        x_s_4 = torch.transpose(x_s_4, 1, 2)
        x_s_4 = x_s_4.view(x_s_4.shape[0], -1, 8, 8)
        ...
        dec_outs = self.decoder(x_f_4, [x_f_3, x_f_2, x_f_1])
        
        # prediction heads
        
        channels =  [768,384,192,96]    
        self.out_head4 = nn.Conv2d(channels[0], 1, 1).to("cuda")
        self.out_head3 = nn.Conv2d(channels[1], 1, 1).to("cuda")
        self.out_head2 = nn.Conv2d(channels[2], 1, 1).to("cuda")
        self.out_head1 = nn.Conv2d(channels[3], 1, 1).to("cuda") 
        p4 = self.out_head4(dec_outs[0]).to("cuda")
        p3 = self.out_head3(dec_outs[1]).to("cuda")
        p2 = self.out_head2(dec_outs[2]).to("cuda")
        p1 = self.out_head1(dec_outs[3]).to("cuda")

        p4 = F.interpolate(p4, scale_factor=32, mode='bilinear').to("cuda")
        p3 = F.interpolate(p3, scale_factor=16, mode='bilinear').to("cuda")
        p2 = F.interpolate(p2, scale_factor=8, mode='bilinear').to("cuda")
        p1 = F.interpolate(p1, scale_factor=4, mode='bilinear').to("cuda")
...
if __name__ =="__main__":
    model = HiFuse_Base(num_classes=1).to("cuda")
    import torch
