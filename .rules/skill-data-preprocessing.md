...
- 涉及数据增强、归一化、重采样等操作
- 修改 `src/preprocess/` 目录下的文件
- 涉及输入验证、图像质量评估、掩码后处理

## 背景知识

医学影像预处理是比赛流水线的关键环节，直接影响模型性能。常见预处理操作包括：
...
```python
from src.preprocess.input_validation import validate_input, detect_input_type
...
```python
from src.preprocess.mask_postprocess import postprocess_mask_summary
...
- `src/preprocess/__init__.py`：预处理模块入口
- `src/preprocess/input_validation.py`：输入验证
- `src/preprocess/image_quality.py`：图像质量评估
- `src/preprocess/ct_preprocess.py`：CT 预处理
- `src/preprocess/mask_postprocess.py`：掩码后处理
- `src/preprocess/roi.py`：感兴趣区域处理
