...
- 涉及分类、检测、分割等评估
- 需要计算准确率、召回率、F1 分数等
- 修改 `osteo_vision_core/metrics/` 目录下的文件
- 涉及指标计算、阈值分析、结果评估

## 背景知识

评估指标是平台框架的核心组件，用于量化模型性能。常见评估指标类型包括：
...
```python
from osteo_vision_core.metrics.segmentation import dice_score, iou_score
...
```python
from osteo_vision_core.metrics.detection import candidate_recall
...
```python
from osteo_vision_core.metrics.classification import classification_metrics
from osteo_vision_core.metrics.segmentation import dice_score, iou_score
...
- `osteo_vision_core/metrics/__init__.py`：指标模块入口
- `osteo_vision_core/metrics/calibration.py`：校准指标
- `osteo_vision_core/metrics/classification.py`：分类指标
- `osteo_vision_core/metrics/detection.py`：检测指标
- `osteo_vision_core/metrics/segmentation.py`：分割指标
