...
| 病例工作台 | JPEG 双通道融合、MP4 播放同步分析、浏览器视频流连续帧串行推理 | MP4 关键帧与逐帧连续分析独立标识 |
...
根目录启动入口会执行严格运行预检、checkpoint sidecar 检查、FFmpeg/ffprobe 检查、后端就绪校验、路由检查和模型预热。独立三维运行时也可单独构建、测试和部署，具体步骤见 [三维运行时说明](docs/three_d_renderer_runtime.md)。
