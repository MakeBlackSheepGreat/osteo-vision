...
test("E2E backend uses the platform strict runtime", async ({ request }) => {
  const response = await request.get(`http://127.0.0.1:${backendPort}/ready`);

  expect(response.ok()).toBeTruthy();
  const payload = await response.json();
  expect(payload.status).toBe("ok");
  expect(payload.runtime_readiness.runtime_profile).toBe("strict_runtime");
  ...
  expect(payload.inference_config).toContain("osteo_vision_strict.yml");
});
