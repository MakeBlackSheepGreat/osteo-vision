  ...
  <section class="anatomy-3d" aria-label="三维颌骨空间证据视图">
    <header class="anatomy-3d__header">
      ...
      <div class="anatomy-3d__status" :class="`is-${riskSummary.risk}`">
        ...
        <small>{{ candidates.length ? `${candidates.length} 个候选区` : "演示证据" }}</small>
      </div>
    </header>
...
const selectedHotspotKey = ref("demo-high");
const modelSourceLabel = ref("示意颌骨模型");
const registrationLabel = ref("演示映射");
...
const demoHotspots: HotspotSpec[] = [
  {
    key: "demo-high",
    label: "右侧下颌体疑似坏死边界区",
    shortLabel: "A",
...
const normalizedHotspots = computed<HotspotSpec[]>(() => {
  if (!props.candidates.length) return demoHotspots;
...
async function findAvailableModel(): Promise<{ path: string } | null> {
  const paths = [
    ...
    "/models/toothfairy_001_label.stl",
    "/models/mandible.stl",
  ];
  for (const path of paths) {
    try {
      ...
      // Missing local model files are expected in the default demo state.
    }
  }
  return null;
}
