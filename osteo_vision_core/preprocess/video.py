...
def extract_keyframes(
    ...
    *,
    ...
    for order, frame_index in enumerate(indexes, start=1):
        ...
        quality = _selected_frame_quality(
            frame,
...
def _selected_frame_quality(
    frame: Any,
    *,
    trace_item: dict[str, Any],
    max_evaluation_side: int | None,
) -> dict[str, Any]:
    """Reuse quality metrics already calculated during quality-peak selection."""

    cached = trace_item.get("quality")
    if isinstance(cached, dict):
        return dict(cached)
    return _frame_quality(frame, max_evaluation_side=max_evaluation_side)
