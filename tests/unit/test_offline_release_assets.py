...
def test_offline_release_assets_define_a_portable_release_layout() -> None:
    root = Path(__file__).resolve().parents[2]
    assets = root / "packaging" / "offline_release"
...
def test_offline_release_builder_checks_media_capacity_and_integrity() -> None:
    root = Path(__file__).resolve().parents[2]
    builder = (root / "scripts" / "build_offline_release_package.ps1").read_text(encoding="utf-8")
