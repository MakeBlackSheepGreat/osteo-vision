...
from tools.build_competition_evidence_index import (
    DEFAULT_MANIFEST,
    ROOT,
    build_payload,
    repository_path,
...
def test_current_evidence_manifest_has_consistent_versions_and_required_files() -> None:
    ...
    assert payload["summary"]["ready_for_submission_freeze"] is True
    assert payload["config"]["path"] == "configs/inference/osteo_vision_competition_strict.yml"
    assert any(
